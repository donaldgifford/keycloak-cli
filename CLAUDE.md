...
Per-repo orientation for `donaldgifford/keycloak-cli`. This file is a
...
`keycloak-cli` is a Go binary maintained as part of the homelab fleet:

- Single binary under `cmd/keycloak-cli/`; library code under
...
- Lives on Forgejo (`github.com/donaldgifford/keycloak-cli`); a
...
```
cmd/keycloak-cli/    # main package — keep thin, parse flags + call into internal/
...
- `just build` — `go build -o bin/keycloak-cli ./cmd/keycloak-cli`
